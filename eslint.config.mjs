...
export default tseslint.config(
  ...
  },
  {
    ignores: [
      ...
      ".vercel/**",
      "next-env.d.ts",
      // Dev scripts (.mjs Node.js scripts) — not Next.js app code.
      // These run outside the Next.js runtime (readline/Fetch/fs/S3 SDK),
      // so the React + Next rules don't apply. typescript-eslint still
      // scans them via the global ts config — exclude explicitly to avoid
      // "unused import" noise from scripts we ship for build-time use.
      "scripts/**",
      // data/ is regenerated by scripts/ (R2 STT + enrichment intermediates).
      // Empty for now but defensively excluded so future runs don't fail lint.
      "data/**",
    ],
  }
);
